...
export function AnalyticsReferrerCard<T extends string>({
  ...
  // Directly calculate maxCount in the render logic
  const maxCount = referrerData?.referrers?.length
    ? Math.max(...referrerData.referrers.map((ref) => ref.clicks))
    : 0;

  console.log("Referrer Data:", referrerData);
  console.log(
    "Mapped Clicks:",
    referrerData?.referrers?.map((ref) => ref.clicks) || []
  );
